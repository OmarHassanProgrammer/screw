<?php

    require "connect_database.php";

    $query = "SELECT * FROM users WHERE token = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param('s', $_POST['token']);
    $stmt->execute();
    $result = $stmt->get_result();

    if($result->num_rows == 0) {
        echo json_encode(array("msg" => "nouser"));
        exit;
    } else {
        $nrow = $result->fetch_assoc();

        $query = "SELECT * FROM room WHERE password = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param('s', $_POST['roomPass']);
        $stmt->execute();
        $result = $stmt->get_result();

        if($result->num_rows == 0) {
            echo json_encode(array("msg" => "noroom"));
            exit;
        } else {
            $row = $result->fetch_assoc();
            
            if($row['turn'] != $nrow['id']) {
                echo json_encode(array("msg" => 'notturn'));
            } else {
                if($row['action'] == "post") {

                    $pastActive = $row["active"];
                    
                    $row['reveal'] = $nrow['id'] . ',' .  $_POST['card'];
                    
                    if($row["active"] >= 7 && $row["active"] <= 10) {
                        $row["active"] = -1;
                        $row["activeStep"] = -1;
                        $row["action"] = "wait";
                    } else if ($row["active"] == 12) {
                        if($row["activeStep"] == 3) {
                            $row["active"] = -1;
                            $row["activeStep"] = -1;
                            $row["action"] = "wait";
                        } else {
                            $row["activeStep"] = $row["activeStep"] + 1;
                        }
                    } else {
                        echo json_encode(array("msg" => "not"));
                        exit;
                    }

                    $query = "UPDATE room SET reveal=?, action=?, active=?, activeStep=?,card=? WHERE password = ?";
                    $stmt = $conn->prepare($query);
                    $stmt->bind_param('ssssss', $row['reveal'], $row['action'], $row['active'], $row['activeStep'], $_POST['card'], $_POST['roomPass']);
                    $stmt->execute();
                    $result = $stmt->get_result();
            
                    if($result) {
                        echo json_encode(array("msg" => 'not'));
                    } else {
                        echo json_encode(array("msg" => "done", "pastActive" => $pastActive));
                    }
                }
            }
        }
    }




?>
